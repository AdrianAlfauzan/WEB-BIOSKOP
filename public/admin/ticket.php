...
<html lang="en">
  ...
  <body class="bg-blue-950 flex">
    ...
    <div class="Admin xl:ms-[300px]">
      ...
      <a href='../pages/tiket/createtiket.php'>
      <button
      ...
      >
        <thead>
          <tr class="">
            <th
              class="border-2 border-slate-950 p-2 w-[100px] text-[10px] xl:text-base xl:w-48"
            >
              NO TICKET
            </th>
            <th
            ...
            <th class="border-2 border-slate-950 p-2 w-[100px] text-[10px] xl:text-base xl:w-48"> ID PENONTON </th>
            <th class="border-2 border-slate-950 p-2 w-[100px] text-[10px] xl:text-base xl:w-48"> TANGGAL </th>
            <th
              class="border-2 border-slate-950 p-2 w-[100px] text-[10px] xl:text-base xl:w-48"
            >
              HARGA
            ...
            <th
              class="border-2 border-slate-950 p-2 w-[] text-[10px] xl:text-base xl:w-48"
            >
              NAMA TEATER
            </th>
            <th
            ...
            <?php
              include('../config/koneksi.php');
                ...
                if (isset($_GET['id_tiket'])) {
                  ...
                  $queryDelete = "DELETE FROM tiket WHERE id_tiket = '$id_tiket'";
            ...
            ?>
              ...
              <?php
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr>
                            <td class='border-2 border-slate-950 p-2 w-80'>{$row['id_tiket']}</td>
                            <td class='border-2 border-slate-950 p-2 w-80'>{$row['id_film']}</td>
                            <td class='border-2 border-slate-950 p-2 w-80'>{$row['id_pegawai']}</td>
                            <td class='border-2 border-slate-950 p-2 w-80'>{$row['id_penonton']}</td>
                            <td class='border-2 border-slate-950 p-2 w-80'>{$row['tanggal']}</td>
                            <td class='border-2 border-slate-950 p-2 w-80'>{$row['harga']}</td>
                            <td class='border-2 border-slate-950 p-2 w-80'>{$row['nama_teater']}</td>
                            <td class='border-2 border-slate-950 p-2'>
                                <div class='flex justify-center mt-4'>
                                <a href='../pages/tiket/updatetiket.php?id=" . $row['id_tiket'] . "'>
                                <button class='rounded-full bg-green-400 xl:bg-slate-600 xl:bg-gradient-to-bl xl:hover:from-sky-100 xl:hover:via-green-600 xl:hover:to-green-500 text-white font-bold text-[10px] p-[2px] mx-[4px] xl:p-1 xl:mx-1 xl:text-base'>
                                  <img src='../Image/update.png' width='30' alt=''>
                                    ...
                                    <a href='?id_tiket={$row['id_tiket']}' onclick='return confirm(\"Apakah Anda yakin ingin menghapus?\")'>
