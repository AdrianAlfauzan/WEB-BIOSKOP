...
<body class="bg-blue-950 flex   ">
    ...
    <div class="Admin xl:ms-[300px]">
        <h1 class="font-bold italic text-white mx-40 text-[28px] text-center xl:mx-60 xl:text-center mt-10 xl:text-5xl">
            ADMIN PANEL</h1>
    <a href='../pages/pegawai/createpegawai.php'>
        <button
        ...
        <table border="3" class="text-white font-bold bg-gradient-to-r from-slate-900 via-blue-950 to-slate-800">
            <thead>
                <tr>
                    <th class="border-2 border-slate-950 p-2 w-[100px] text-[10px] xl:text-base xl:w-80">ID PEGAWAI</th>
                    <th class="border-2 border-slate-950 p-2 w-80 text-[10px] xl:text-base xl:w-80">NAMA</th>
                    <th class="border-2 border-slate-950 p-2 w-80 text-[10px] xl:text-base xl:w-80">JOBDESK</th>
                    <th class="border-2 border-slate-950 p-2 w-80 text-[10px] xl:text-base xl:w-80">Aksi</th>
                </tr>
            </thead>
            <tbody>
            <?php
                ...
                if (isset($_GET['id_pegawai'])) {
                    ...
                    // Query untuk menghapus data pegawai berdasarkan id_pegawai
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr>
                            ...
                            <td class='border-2 border-slate-950 p-2 w-80'>{$row['nama']}</td>
                            <td class='border-2 border-slate-950 p-2 w-80'>{$row['jobdesk']}</td>
                            <td class='border-2 border-slate-950 p-2'>
                            <div class='flex justify-center items-center h-full mt-4'>
                                <a href='../pages/pegawai/updatepegawai.php?id=" . $row['id_pegawai'] . "'>
                                ...
                                <a href='?id_pegawai={$row['id_pegawai']}' onclick='return confirm(\"Apakah Anda yakin ingin menghapus?\")'>
