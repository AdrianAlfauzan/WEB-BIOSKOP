...
<html lang="en">
  ...
  <body class="bg-blue-950 flex">
    ...
    <div class="Admin xl:ms-[300px]">
      ...
      <a href='../pages/genre/creategenre.php'>
        <button
      ...
      >
        <thead>
          <tr class="">
            <th
              class="border-2 border-slate-950 p-2 w-[] text-[10px] xl:text-base xl:w-80"
            >
              JENIS GENRE
            </th>
            <th
              class="border-2 border-slate-950 p-2 w-[100px] text-[10px] xl:text-base xl:w-80"
            >
              ID GENRE
            </th>
            <th
        ...
        <?php
          ...
          if (isset($_GET['id_film'])) {
            ...
            $queryDelete = "DELETE FROM genre WHERE id_film = '$id_film'";
        ...
        <?php
            $result = mysqli_query($koneksi, "SELECT * FROM genre");
            while ($row = mysqli_fetch_assoc($result)) {
                echo "<tr>
                        <td class='border-2 border-slate-950 p-2 w-80'>{$row['jenis']}</td>
                        <td class='border-2 border-slate-950 p-2 w-80'>{$row['id_genre']}</td>
                        <td class='border-2 border-slate-950 p-2 w-80'>{$row['id_film']}</td>
                        <td class='border-2 border-slate-950 p-2'>
                            <div class='flex justify-center mt-4'>
                            <a href='../pages/genre/updategenre.php?id=" . $row['id_film'] . "'>
